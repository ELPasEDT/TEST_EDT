...
Процедура ЗаполнитьНаборыСвойствОбъекта(Объект, ТипСсылки, НаборыСвойств, СтандартнаяОбработка, КлючНазначения) Экспорт
	...
	Если ТипСсылки = Тип("СправочникСсылка.Оборудование") Тогда
        ЗаполнитьНаборСвойствПоВидуОборудования(
...
Процедура ЗаполнитьНаборСвойствПоВидуОборудования(Объект, ТипСсылки, НаборыСвойств)
	
	Строка = НаборыСвойств.Добавить();
	//Строка.Набор = Справочники.НаборыДополнительныхРеквизитовИСведений.Справочник_Оборудование;
	//Строка.ОбщийНабор = ЛОжь;
	
	Если ТипЗнч(Объект) = ТипСсылки Тогда
		Оборудование = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			Объект, "ВидОборудования");
	Иначе
		Оборудование = Объект;
	КонецЕсли;
	...
	Строка.Набор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		Оборудование.ВидОборудования, "НаборСвойств");
