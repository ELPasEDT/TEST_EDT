...
Процедура НастроитьФорму()
	...
	Если ДанныеРегистраНастроек.Количество() = 0 Тогда
		ЭтаФорма.Элементы.ЗначенияПоказателей.Видимость = Ложь;
	КонецЕсли;
	
	//Убираем видимость таблиц
	Если ДанныеРегистраНастроекШапка.Количество() = 0 Тогда
		ЭтаФорма.Элементы.ПоказателиШапки.Видимость = Ложь;
	КонецЕсли;
...
Функция ПолучитьДанныеРегистраНастроек()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	НастройкаЗначенийПоказателейГлавнаяСхема.ЗначениеПоказателя КАК ЗначениеПоказателя,
		|	НастройкаЗначенийПоказателейГлавнаяСхема.Синоним КАК Синоним,
		|	НастройкаЗначенийПоказателейГлавнаяСхема.ТипЗначения КАК ТипЗначения
		|ИЗ
		|	РегистрСведений.НастройкаЗначенийПоказателейГлавнаяСхема КАК НастройкаЗначенийПоказателейГлавнаяСхема
		|ГДЕ
		|	НастройкаЗначенийПоказателейГлавнаяСхема.Организация = &Организация";
...
Функция ПолучитьДанныеРегистраНастроекШапка()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	НастройкаЗначенийПоказателейГлавнаяСхемаШапка.ЗначениеПоказателя КАК ЗначениеПоказателя,
		|	НастройкаЗначенийПоказателейГлавнаяСхемаШапка.Синоним КАК Синоним,
		|	НастройкаЗначенийПоказателейГлавнаяСхемаШапка.ТипЗначения КАК ТипЗначения
		|ИЗ
		|	РегистрСведений.НастройкаЗначенийПоказателейГлавнаяСхемаШапка КАК НастройкаЗначенийПоказателейГлавнаяСхемаШапка
		|ГДЕ
		|	НастройкаЗначенийПоказателейГлавнаяСхемаШапка.Организация = &Организация";
