...
Процедура СоздатьПоШаблону(Команда)
	...
	ОткрытьФорму("Документ.мРапортНСКТЦ.ФормаОбъекта", ПараметрыФормы);
...
Функция ПолучитьДокументШаблона()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ШаблонРапортНСКТЦ.Ссылка КАК Ссылка
		|ИЗ
		|	Документ.ШаблонРапортНСКТЦ КАК ШаблонРапортНСКТЦ
		|ГДЕ
		|	ШаблонРапортНСКТЦ.Проведен
		|	И ШаблонРапортНСКТЦ.Организация = &Организация
		|	И ШаблонРапортНСКТЦ.Подразделение = &Подразделение
		|
		|УПОРЯДОЧИТЬ ПО
		|	ШаблонРапортНСКТЦ.Дата УБЫВ";
	...
	Иначе
		Возврат Документы.ШаблонРапортНСКТЦ.ПустаяСсылка();
	КонецЕсли;
